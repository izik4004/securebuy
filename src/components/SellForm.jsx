...
const SellForm = ({modalOpen, setModalOpen}) => {
  return (
    <div 
    show={modalOpen}
    // onHide={() => setItemModal(false)}
    class="py-12 bg-transparent transition duration-50 ease-in-out z-10 absolute top-0 right-0 bottom-0 left-0" 
    id="modal">
    <div role="alert" class=" mx-auto w-8/12 md:w-2/3 max-w-md">
        <div class="relative py-8 px-5 md:px-10 bg-white shadow rounded-xl">
            ...
            <h1 class="text-gray-800 font-lg font-bold tracking-normal leading-tight mb-4">Enter Product Details</h1>
            ...
            <label for="email2" class="text-gray-800 text-sm font-bold leading-tight tracking-normal">Price</label>
            ...
            <div class="flex items-center justify-start w-full">
                ...
                <button class="focus:outline-none focus:ring-2 focus:ring-offset-2  focus:ring-gray-400 ml-3 bg-gray-100 transition duration-150 text-gray-600 ease-in-out hover:border-gray-400 hover:bg-gray-300 border rounded px-8 py-2 text-sm"  onClick={() =>setModalOpen(false)} >Cancel</button>
            </div>
            <button onClick={() => setModalOpen(false)} class="cursor-pointer absolute top-0 right-0 mt-4 mr-5 text-gray-400 hover:text-gray-600 transition duration-150 ease-in-out rounded focus:ring-2 focus:outline-none focus:ring-gray-600" onclick="modalHandler()" aria-label="close modal" role="button">
